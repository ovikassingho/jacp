...
public abstract class AFXWorkbench implements IWorkbench<Node, EventHandler<Event>, Event, Object>, IRootComponent<IPerspective<EventHandler<Event>, Event, Object>, IAction<Event, Object>> {
    ...
    public final void start(final Stage stage) throws Exception {
        ...
        this.handleInitialLayout(new FXAction("TODO", "init"), this.getWorkbenchLayout());
        this.setBasicLayout(stage);

        this.postHandle(new FXComponentLayout(this.getWorkbenchLayout().getMenu(), this.getWorkbenchLayout().getRegisteredToolbars(), this.glassPane));

        this.log("3: handle initialisation sequence");
        this.componentHandler = new FXWorkbenchHandler(this.launcher, this.workbenchLayout, this.root, this.perspectives);
            ...
            if (perspective.isActive()) {
                Platform.runLater(new Runnable() {
                    ...
                    public final void run() {
                        AFXWorkbench.this.componentHandler.initComponent(new FXAction(perspective.getId(), perspective.getId(), "init"), perspective);
                    }
                }); // FX2 UTILS END
            }
    ...
    private void handleInitialisationSequence() {
        Platform.runLater(new Runnable() {
            ...
            public void run() {
                ...
                ((FXPerspectiveCoordinator) AFXWorkbench.this.perspectiveCoordinator).start();
                ((FXComponentDelegator) AFXWorkbench.this.componentDelegator).start();
                ((FXMessageDelegator) AFXWorkbench.this.messageDelegator).start();
                // handle perspectives
    ...
    public final void registerComponent(final IPerspective<EventHandler<Event>, Event, Object> perspective) {
        ...
        perspective.init(this.componentDelegator.getComponentDelegateQueue(), this.messageDelegator.getMessageDelegateQueue(), this.perspectiveCoordinator.getMessageQueue());
     ...
     * @param component
     */
    private void handleMetaAnnotation(final IPerspective<EventHandler<Event>, Event, Object> perspective) {
        final Perspective perspectiveAnnotation = perspective.getClass().getAnnotation(Perspective.class);
        if (perspectiveAnnotation != null) {
            final String id = perspectiveAnnotation.id();
            if (id != null)
                FXUtil.setPrivateMemberValue(AComponent.class, perspective, FXUtil.ACOMPONENT_ID, id);
            FXUtil.setPrivateMemberValue(AComponent.class, perspective, FXUtil.ACOMPONENT_ACTIVE, perspectiveAnnotation.active());
            final String name = perspectiveAnnotation.name();
            if (name != null)
                FXUtil.setPrivateMemberValue(AComponent.class, perspective, FXUtil.ACOMPONENT_NAME, name);
            this.log("register perspective with annotations : " + perspectiveAnnotation.id());
            final String viewLocation = perspectiveAnnotation.viewLocation();
            if (viewLocation.length() > 1)
                FXUtil.setPrivateMemberValue(AFXPerspective.class, perspective, FXUtil.IDECLARATIVECOMPONENT_VIEW_LOCATION, perspectiveAnnotation.viewLocation());
            if (viewLocation.length() > 1)
                FXUtil.setPrivateMemberValue(AFXPerspective.class, perspective, FXUtil.IDECLARATIVECOMPONENT_TYPE, UIType.DECLARATIVE);
            final String localeID = perspectiveAnnotation.localeID();
            if (localeID.length() > 1)
                FXUtil.setPrivateMemberValue(AFXPerspective.class, perspective, FXUtil.IDECLARATIVECOMPONENT_LOCALE, localeID);
            final String resourceBundleLocation = perspectiveAnnotation.resourceBundleLocation();
            if (resourceBundleLocation.length() > 1)
                FXUtil.setPrivateMemberValue(AFXPerspective.class, perspective, FXUtil.IDECLARATIVECOMPONENT_BUNDLE_LOCATION, resourceBundleLocation);
        }
    }
    ...
    public final IActionListener<EventHandler<Event>, Event, Object> getActionListener(final String targetId, final Object message) {
        return new FXActionListener(new FXAction("workbench", targetId, message), this.perspectiveCoordinator.getMessageQueue());
    }
    ...
    private void initToolbarLayout() {
        ...
        if (!this.getWorkbenchLayout().getRegisteredToolbars().isEmpty()) {
            ...
            toolbarPane.getStyleClass().add(CSSUtil.CSSConstants.CLASS_DARK_BORDER);

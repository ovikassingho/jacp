...
public abstract class ASwingWorkbench extends JFrame
	...
	public void initWorkbenchMenu() {
		...
		//TODO handle default menu entries for different OS
	...
	private void runReassign(
			final IPerspectiveLayout<? extends Container, Container> layout,
			final IPerspective<ActionListener, ActionEvent, Object> perspective) {
		final List<ISubComponent<ActionListener, ActionEvent, Object>> subcomponents = perspective
				.getSubcomponents();
		for (int i = 0; i < subcomponents.size(); i++) {
			final ISubComponent<ActionListener, ActionEvent, Object> subComp = subcomponents
					.get(i);
			if (subComp instanceof ASwingComponent) {
				...
				if (editorComponent != null) {
					...
					addComponentByType(layout, ((ASwingComponent) subComp));
				}
			}
		}
	}
	 ...
	 * @param newComp
	 */
	private void reassignChild(final Container parent, final Container oldComp,
			final Container newComp) {

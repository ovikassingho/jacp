...
public abstract class SwingWorker<T, V> implements RunnableFuture<T> {
     ...
     */
    private static final int MAX_WORKER_THREADS = 30;
	...
	synchronized (this) {
		...
		doNotifyProgressChange = new AccumulativeRunnable<Integer>() {
			...
			firePropertyChange("progress", args.get(0),
	...
	ExecutorService executorService = (ExecutorService) appContext
		.get(SwingWorker.class);
	if (executorService == null) {
	    ...
	    appContext.addPropertyChangeListener(
			...
			public void propertyChange(final PropertyChangeEvent pce) {
			    final boolean disposed = (Boolean) pce
				    .getNewValue();
			    if (disposed) {

...
public abstract class ASwingWorkbench extends JFrame
	 ...
	 */
        ...
        public Container init(final Launcher<?> launcher) {
                ...
                this.handleInitialLayout(new SwingAction("TODO", "init"),
                                layout);
        ...
        /**
         * handles sequence for workbench size, menu bar, tool bar and
        ...
        private void handleInitialisationSequence() {
                SwingUtilities.invokeLater(new Runnable() {
                        ...
                        public void run() {
                                ...
                                ((SwingPerspectiveCoordinator) perspectiveHandler)
                                                .start();
                                // init size
                                initWorkbenchSize();
                                // init menu instance#
        ...
        private void initToolBars() {
                final Iterator<Entry<Layout, Container>> it = layout
                                .getToolBars().entrySet().iterator();
                while (it.hasNext()) {
                        final Entry<Layout, Container> entry = it.next();
                        getContentPane().add(entry.getValue(),
                                        entry.getKey().getLayout());
                }
        ...
        private void createPerspectiveMenue(
                ...
                if (perspective instanceof ASwingPerspective) {
                        ...
                        ((ASwingPerspective) perspective)
                                        .handleMenuEntries(tmp);
                        if (getJMenuBar() != null
        ...
        /**
         * perspectives are a set in workbench. Each perspective contains a
         * Layout object (container like splitPane), a view container and a
                ...
                if (!SwingUtilities.isEventDispatchThread()) {
                        try {
                                SwingUtilities.invokeAndWait(new Runnable() {
                                        @Override
                                        public void run() {
                                                runReassign(layout, perspective);
        ...
        /**
         * takes the root component from old container, removes old container
         * and add the new one
         * 
         * @param oldComp
         * @param newComp
        ...
        private Container getParentAndReplace(final Container oldComp,
                ...
                if (!SwingUtilities.isEventDispatchThread()) {
                        ...
                        reassignChild(parent, oldComp, newComp);
                }
                return parent;
        }
         ...
         * @param newComp
         */
        private void reassignChild(final Container parent,
                        final Container oldComp, final Container newComp) {
        ...
        /**
         * initialize perspective in window mode; creates an internal frame and
         * add perspective
         * 
         * @param layout
         * @param name
        ...
        private void initPerspectiveInWindowMode(
                ...
                final Container comp = getLayoutComponentFromPerspectiveLayout(
                                layout, new Dimension(
                                                this.layout.getWorkbenchSize()
                                                                .getX() - 30,
                                                this.layout.getWorkbenchSize()
                                                                .getY() - 20));
                frame.add(comp);
        ...
        /**
         * initialize perspective in tabbed mode; creates an tab an add
         * perspective
         * 
         * @param layout
         */
        ...
        private Dimension getPanelDimension(
                        final IWorkbenchLayout<LayoutManager2, Container> layout) {
                return new Dimension(layout.getWorkbenchSize().getX() - 15,
                                layout.getWorkbenchSize().getY() - 15);
        }
        ...
        private void putComponents(
                        final IPerspectiveLayout<? extends Container, Container> layout,
                        final IVComponent<Container, ActionListener, ActionEvent, Object> editor) {
                final Container validContainer = layout
                                .getTargetLayoutComponents().get(
                                                editor.getExecutionTarget());
                if (validContainer instanceof JScrollPane) {
                        ((JScrollPane) validContainer).setViewportView(editor
                                        .getRoot());
                } else {
        ...
        private JInternalFrame getActiveInternalFrame(final String name) {
                final JInternalFrame internalFrame = new JInternalFrame(name,
                                true, true, true, true);
                try {
                        internalFrame.setSelected(true);
                } catch (final PropertyVetoException ex) {
                        Logger.getLogger(ASwingWorkbench.class.getName()).log(
                                        Level.SEVERE, null, ex);
                }
                internalFrame.setSize(layout.getWorkbenchSize().getX() - 10,
                                layout.getWorkbenchSize().getY() - 10);

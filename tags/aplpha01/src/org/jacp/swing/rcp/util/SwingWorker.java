 ...
 * executed. Then {@code SwingWorker} notifies any {@code
 * PropertyChangeListeners} about the {@code state} property change to {@code
 * StateValue.DONE}.
 * 
 * <p>
 * {@code SwingWorker} is only designed to be executed once. Executing a {@code
 * SwingWorker} more than once will not result in invoking the {@code
 ...
 * 
 * <p>
 * Say we want to find the "Meaning of Life" and display the result in a {@code
 * JLabel}.
 * 
 * <pre>
 *   final JLabel label;
 ...
 * 
 * <p>
 * Because {@code SwingWorker} implements {@code Runnable}, a {@code
 * SwingWorker} can be submitted to an {@link java.util.concurrent.Executor} for
 * execution.
 * 
 ...
 * 
 * @param <T>
 *            the result type returned by this {@code SwingWorker's} {@code
 *            doInBackground} and {@code get} methods
 * @param <V>
...
public abstract class SwingWorker<T, V> implements RunnableFuture<T> {
	...
	public enum StateValue {
		...
		PENDING,
		/**
		 * {@code SwingWorker} is {@code STARTED} before invoking {@code
		 * doInBackground}.
		 */
		STARTED,
	 ...
	 * 
	 */
	protected final void publish(final V... chunks) {
		synchronized (this) {
			if (doProcess == null) {
				doProcess = new AccumulativeRunnable<V>() {
					@Override
					public void run(final List<V> args) {
						process(args);
					}
	 ...
	 * the <i>Event Dispatch Thread</i> multiple invocations to the {@code
	 * setProgress} method might occur before any {@code
	 * PropertyChangeListeners} are invoked. For performance purposes all these
	 * invocations are coalesced into one invocation with the last invocation
	 * argument only.
	 * 
	 * <p>
		...
		synchronized (this) {
			if (doNotifyProgressChange == null) {
				doNotifyProgressChange = new AccumulativeRunnable<Integer>() {
					@Override
					public void run(final List<Integer> args) {
						firePropertyChange("progress", args.get(0), args
								.get(args.size() - 1));
					}
	 ...
	 * event all <i>worker</i> threads are busy handling other {@code
	 * SwingWorkers} this {@code SwingWorker} is placed in a waiting queue.
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * When called off the <i>Event Dispatch Thread</i> {@code
	 * PropertyChangeListeners} are notified asynchronously on the <i>Event
	 * Dispatch Thread</i>.
	 * <p>
	 * Note: This is merely a convenience wrapper. All work is delegated to
	 * {@code PropertyChangeSupport} from {@link #getPropertyChangeSupport}.
	 ...
	 * 
	 * <p>
	 * Note: The returned {@code PropertyChangeSupport} notifies any {@code
	 * PropertyChangeListener}s asynchronously on the <i>Event Dispatch
	 * Thread</i> in the event that {@code firePropertyChange} or {@code
	 * fireIndexedPropertyChange} are called off the <i>Event Dispatch
	 * Thread</i>.
	 * 
	 ...
	 * If the last one it triggers autoShutdown thread to get started.
	 * 
	...
	private static synchronized ExecutorService getWorkersExecutorService() {
		...
		if (obj == null) {
			 ...
			 */
			obj = new ThreadPoolExecutor(MAX_WORKER_THREADS,
					MAX_WORKER_THREADS, 1L, TimeUnit.SECONDS,
				...
				public void execute(final Runnable command) {
					/*
					 * ThreadPoolExecutor first tries to run task in a corePool.
					 * If all threads are busy it tries to add task to the
					 * waiting queue. If it fails it run task in maximumPool.
					 * 
					 * We want corePool to be 0 and maximumPool to be
					 * MAX_WORKER_THREADS We need to change the order of the
					 * execution. First try corePool then try maximumPool pool
					 * and only then store to the waiting queue. We can not do
					 * that because we would need access to the private methods.
					 * 
					 * Instead we enlarge corePool to MAX_WORKER_THREADS before
					 * the execution and shrink it back to 0 after. It does
					 * pretty much what we need.
					 * 
					 * While we changing the corePoolSize we need to stop
					 * running worker threads from accepting new tasks.
					 */
	...
	private static class DoSubmitAccumulativeRunnable extends
		...
		@Override
		protected void run(final List<Runnable> args) {
			for (final Runnable runnable : args) {
				runnable.run();
			}
		}
	...
	private class SwingWorkerPropertyChangeSupport extends
		 ...
		 * 
		 */
		private static final long serialVersionUID = -8242478452302262989L;

		SwingWorkerPropertyChangeSupport(final Object source) {
			super(source);
		}
